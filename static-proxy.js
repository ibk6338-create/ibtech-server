...
function normalizeHtml(html){return html.replace(/Epin Manager/gi,'ePINs').replace(/ePIN Manager/gi,'ePINs').replace(/EPIN MANAGER/gi,'ePINs').replace(/images\/branding\/ib-tech-epins-logo\.webp/g,'images/branding/ib-tech-epins-logo.svg');}
const server=http.createServer((req,res)=>{if(req.url.startsWith('/api/'))return proxy(req,res);if(req.method!=='GET'){res.writeHead(405);return res.end('Method Not Allowed')};let u=req.url.split('?')[0];if(u==='/')u='/index.html';const f=path.normalize(path.join(root,u));if(!f.startsWith(root+path.sep)||!fs.existsSync(f)||!fs.statSync(f).isFile()){res.writeHead(404);return res.end('Not found')};const ext=path.extname(f).toLowerCase();if(ext==='.html'){const html=normalizeHtml(fs.readFileSync(f,'utf8'));res.writeHead(200,{'Content-Type':mime[ext]});return res.end(html)}res.writeHead(200,{'Content-Type':mime[ext]||'application/octet-stream'});fs.createReadStream(f).pipe(res)});
server.on('error',e=>{console.error('IB-TECH web gateway error:',e);process.exit(1)});
server.listen(externalPort,'0.0.0.0',()=>console.log(`IB-TECH web gateway ready on 0.0.0.0:${externalPort}`));
